...
export function Navigation() {
  ...
  const [userRole, setUserRole] = useState<"brand" | "user" | null>(null);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const role = localStorage.getItem("userRole") as "brand" | "user" | null;
      setUserRole(role);
    }
  }, [pathname]);
    ...
    <nav className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-16 items-center justify-between px-4">
        ...
        <div className="flex items-center gap-2">
              ...
              <Button
                ...
                className={`hidden md:inline-flex rounded-full transition-all ${
                  pathname === "/brands"
                    ...
                    : "bg-transparent hover:bg-primary/10 hover:border-primary"
                }`}
              >
                <Briefcase
          ...
          <Link href="/profile">
            <Avatar
              className={`h-9 w-9 cursor-pointer ring-2 transition-all ${
                pathname === "/profile"
                  ? "ring-primary"
              ...
              <AvatarFallback
                className={`transition-colors ${
                  pathname === "/profile"
                    ...
                    : "bg-purple-200 text-purple-600 hover:bg-primary/10 hover:text-primary"
                }`}
              >
